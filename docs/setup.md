...
Copy `kimodo_server.py` from this repo into the `kimodo/` directory:
...
In live mode each `/generate` call runs full Kimodo diffusion on GPU (30 s – 3 min depending on duration). The first call also downloads model weights from HuggingFace (~10 min on first run).

---

## Step 5: Connect from Houdini (Python Shell)
...
Install in Houdini — see [hda/README.md](../hda/README.md) for full HDA documentation and install options.

In any SOP network, drop a **kimodo_motion** node. Set **Host Output Dir** to the path of your `kimodo/output/` directory, then press **Generate**.
